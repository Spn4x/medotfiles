...
KITTY_LIGHTEN_FACTOR="1.5" # Lighten colors by 70%. Adjust this value as you like.
# --- KITTY END ---
        ...
        if [ -x "$HELPER_SCRIPT_PATH" ]; then
            if [ -n "$background" ] && [ -n "$foreground" ] && [ -n "$cursor" ] && \
               ...
               [ -n "$color12" ] && [ -n "$color13" ] && [ -n "$color14" ] && [ -n "$color15" ]; then
                ...
                # Call the helper script for each color
                l_background=$("$HELPER_SCRIPT_PATH" "$background" "$KITTY_LIGHTEN_FACTOR")
                l_foreground=$("$HELPER_SCRIPT_PATH" "$foreground" "$KITTY_LIGHTEN_FACTOR")
                l_cursor=$("$HELPER_SCRIPT_PATH" "$cursor" "$KITTY_LIGHTEN_FACTOR")
                l_color0=$("$HELPER_SCRIPT_PATH" "$color0" "$KITTY_LIGHTEN_FACTOR")
                l_color1=$("$HELPER_SCRIPT_PATH" "$color1" "$KITTY_LIGHTEN_FACTOR")
                l_color2=$("$HELPER_SCRIPT_PATH" "$color2" "$KITTY_LIGHTEN_FACTOR")
                l_color3=$("$HELPER_SCRIPT_PATH" "$color3" "$KITTY_LIGHTEN_FACTOR")
                l_color4=$("$HELPER_SCRIPT_PATH" "$color4" "$KITTY_LIGHTEN_FACTOR")
                l_color5=$("$HELPER_SCRIPT_PATH" "$color5" "$KITTY_LIGHTEN_FACTOR")
                l_color6=$("$HELPER_SCRIPT_PATH" "$color6" "$KITTY_LIGHTEN_FACTOR")
                l_color7=$("$HELPER_SCRIPT_PATH" "$color7" "$KITTY_LIGHTEN_FACTOR")
                l_color8=$("$HELPER_SCRIPT_PATH" "$color8" "$KITTY_LIGHTEN_FACTOR")
                l_color9=$("$HELPER_SCRIPT_PATH" "$color9" "$KITTY_LIGHTEN_FACTOR")
                l_color10=$("$HELPER_SCRIPT_PATH" "$color10" "$KITTY_LIGHTEN_FACTOR")
                l_color11=$("$HELPER_SCRIPT_PATH" "$color11" "$KITTY_LIGHTEN_FACTOR")
                l_color12=$("$HELPER_SCRIPT_PATH" "$color12" "$KITTY_LIGHTEN_FACTOR")
                l_color13=$("$HELPER_SCRIPT_PATH" "$color13" "$KITTY_LIGHTEN_FACTOR")
                l_color14=$("$HELPER_SCRIPT_PATH" "$color14" "$KITTY_LIGHTEN_FACTOR")
                l_color15=$("$HELPER_SCRIPT_PATH" "$color15" "$KITTY_LIGHTEN_FACTOR")

                # Write the new theme file
                cat > "$KITTY_THEME_OUTPUT" << EOF
# Kitty colors generated by wallpaper.sh
# Theme: Lighter variant from wallust
...
color15             $l_color15
EOF
                echo "Kitty theme written to $KITTY_THEME_OUTPUT"
            else
                echo "Warning: Kitty color vars not fully loaded. Skipping Kitty theme."
            fi
        else
            echo "Error: Kitty helper script not found or not executable at $HELPER_SCRIPT_PATH."
        fi
        # --- KITTY THEME END ---
...
if [ -f "$VICINAE_THEME_TEMPLATE" ]; then
    echo "Generating Vicinae dynamic theme..."
    tmp_vicinae_json=$(mktemp)
    cp "$VICINAE_THEME_TEMPLATE" "$tmp_vicinae_json"

    # Replace all placeholders
    sed -i "s|{{background}}|$background|g" "$tmp_vicinae_json"
    sed -i "s|{{foreground}}|$foreground|g" "$tmp_vicinae_json"
    sed -i "s|{{color1}}|$color1|g" "$tmp_vicinae_json"
    sed -i "s|{{color2}}|$color2|g" "$tmp_vicinae_json"
    sed -i "s|{{color3}}|$color3|g" "$tmp_vicinae_json"
    sed -i "s|{{color4}}|$color4|g" "$tmp_vicinae_json"
    sed -i "s|{{color5}}|$color5|g" "$tmp_vicinae_json"
    sed -i "s|{{color6}}|$color6|g" "$tmp_vicinae_json"

    mv "$tmp_vicinae_json" "$VICINAE_THEME_OUTPUT"
    echo "Vicinae theme written to $VICINAE_THEME_OUTPUT"
else
    echo "Error: Vicinae template $VICINAE_THEME_TEMPLATE not found."
fi

 # --- Vicinae Dynamic Theme END ---
