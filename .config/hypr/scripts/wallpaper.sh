...
GLAVA_RADIAL_SHADER_FILE="$HOME/.config/glava/radial.glsl"
# --- SWAYNC START ---
SWAYNC_STYLE_BASE="$HOME/.config/swaync/style-base.css"
SWAYNC_STYLE_OUTPUT="$HOME/.config/swaync/style.css"
# --- SWAYNC END ---
# --- CHEATSHEET START ---
CHEATSHEET_STYLE_TEMPLATE="/home/meismeric/.config/hypr/python/cheatsheet-template.css"
CHEATSHEET_STYLE_OUTPUT="/home/meismeric/.config/hypr/python/cheatsheet.css"
# --- CHEATSHEET END ---
...
function M.setup()
    ...
    hi("Directory", { fg =# --- CHEATSHEET START ---
...
# --- CHEATSHEET END --- colors.color4 }); hi("DiffAdd", { bg = colors.color2, fg = colors.background })
...
EOF
        ...
        # --- GLAVA SECTION START (Targeting specific shaders) ---
...
# --- Interactive Mode (wofi) ---
echo "Interactive mode: Selecting new wallpaper."
selected_basename=$(echo "$WALLPAPER_FILES" | while read -r file; do echo "$(basename "$file")"; done | wofi --dmenu --allow-images --insensitive --prompt "$WOFI_PROMPT" --style "$WOFI_STYLE_OUTPUT")
...
SELECTED_NEW_WALLPAPER_PATH=$(echo "$WALLPAPER_FILES" | grep -m 1 "/${selected_basename}$")

if [ -z "$SELECTED_NEW_WALLPAPER_PATH" ] || [ ! -f "$SELECTED_NEW_WALLPAPER_PATH" ]; then
    send_notification -u critical "Wallpaper Script Error" "Could not find path for '$selected_basename'."
    exit 1
fi
